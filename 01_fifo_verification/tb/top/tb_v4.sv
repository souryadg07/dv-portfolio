...
module tb_sync_fifo_v4;
  ...
  initial begin
    ...
    begin
      semaphore                           wr_sem,rd_sem;
      fifo_wr_driver #(DATA_WIDTH, DEPTH) wdrv;
      fifo_rd_driver #(DATA_WIDTH, DEPTH) rdrv;

      wr_sem = new(1);  // one token = mutual exclusion
      rd_sem = new(1);  // one token = mutual exclusion
      wdrv    = new(fif, wr_sem, 50);
      rdrv    = new(fif, rd_sem, 50);
